...
const ReceiptPage = () => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (classificationResult.type !== 'invoice' && classificationResult.type !== 'receipt') {
        navigate(`/review_result?success=false&storeName=${encodeURIComponent(storeName)}&storeAddress=${encodeURIComponent(storeAddress)}&storeId=${storeId}`)
        return
      }
      ...
      navigate(`/review_result?success=true&storeName=${encodeURIComponent(storeName)}&storeAddress=${encodeURIComponent(storeAddress)}&storeId=${storeId}`)
    } catch (error) {
  ...
  return (
    <motion.div
      ...
      <Information />
      {/* 3 */}
      <h5 className='pt-8 px-4 font-bold text-2xl'>구매 인증 방식</h5>
      <div className='grid grid-cols-2 gap-4 pt-5 w-full justify-items-center'>
        ...
        <button
          ...
          <svg
            width='30'
            height='30'
            className='mb-2 text-gray-500 group-hover:text-[#FC7E2A] transition-colors duration-300 sm:w-[35px] sm:h-[35px] md:w-10 md:h-10'
            fill='currentColor'
            viewBox='0 0 40 40'
      ...
      {preview && (
        <div className='mt-8 px-4'>
          <div className='flex items-center justify-between mb-3'>
            <p className='text-lg font-semibold text-gray-800'>업로드한 영수증</p>
            <button
              onClick={() => {
                setPreview(null)
                setIsUploading(false)
          ...
          <div className='relative w-full bg-gray-50 rounded-lg overflow-hidden shadow-md'>
            <img
              src={preview}
              alt='영수증 미리보기'
            ...
            {isUploading && (
              <div className='absolute inset-0 bg-black/50 flex items-center justify-center'>
                <div className='flex flex-col items-center gap-4'>
                  <SpoonLoader />
                  <p className='text-white font-semibold text-lg'>영수증 검증 중...</p>
                </div>
              </div>
            )}
          </div>
      ...
      <div className='px-4 mt-30'>
        <button
          onClick={() => navigate(`/review_result?success=true&storeName=${encodeURIComponent(storeName)}&storeAddress=${encodeURIComponent(storeAddress)}&storeId=${storeId}`)}
          className='cursor-pointer w-full py-3 bg-green-500 text-white font-bold rounded-lg hover:bg-green-600 active:bg-green-700 transition-colors'>
          영수증 인증 성공 (테스트용)
        </button>
      </div>
    </motion.div>
