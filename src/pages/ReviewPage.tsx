...
const ReviewPage = () => {
  ...
  //팝업 
  const handleReviewSubmit = () => { setIsPopupOpen(true); };
  ...
  return (
    <div className='container mx-auto p-4 pt-10'>
      ...
      <div className="mb-6">
        ...
        <div className="flex space-x-4">
          <div className="flex items-center">
            ...
            {/* 사진 등록 버튼 임시 디자인 */}
            <label htmlFor="image-upload" className="border border-gray-300 p-4 rounded-md cursor-pointer">
      ...
      <div className="text-center">
        <button onClick={handleReviewSubmit} className="bg-[#FC7E2A] text-white py-3 px-6 rounded-md font-semibold w-full mt-6">리뷰 등록</button>
      </div>
      {isPopupOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-[999]" style={{ backgroundColor: 'rgba(0, 0, 0, 0.85)' }} >
          <div className="bg-white p-6 rounded-lg w-11/12 max-w-sm shadow-2xl z-[1000]">
            ...
            <p className="text-sm text-gray-700 text-center mt-4 mb-6 leading-relaxed">
              리뷰는 나중에 삭제하거나 수정할 수 없어요.<br/>
              신중하게 작성해 주세요.
            </p>
            <div className="flex justify-between mt-6 space-x-3">
              <button onClick={handlePopupCancel} className="flex-1 bg-white text-gray-700 font-semibold py-3 px-6 rounded-md border border-gray-300">
                취소
              </button>
              <button onClick={handlePopupConfirm} className="flex-1 bg-orange-400 text-white font-semibold py-3 px-6 rounded-md">
                확인
              </button>
            </div>
          </div>
