        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-3 gap-3 gap-y-4 text-[#444444]">
            {['맛있었어요', '혼밥하기 좋아요', '양이 많아요'].map((text) => (
              <button key={text} onClick={() => handleButtonClick(text)} className={`border px-4 py-2.5 rounded-md text-center text-sm 
                ${selectedButtons.includes(text)
                  ? 'border-[#FC7E2A] text-[#FC7E2A] bg-white font-semibold'
                  : 'bg-gray-100 border-gray-200 font-semibold'}`}
              >
                {text}
              </button>
            ))}
      ...
      <div className='mb-6'>
        ...
        <div className='flex space-x-4'>
          <div className='flex flex-col items-center border py-4 px-6 rounded-md'>
            <input type='file' className='hidden' id='image-upload' onChange={handleImageUpload} multiple/>
            <div className="relative w-6 h-6">
              {/* 사진 아이콘 */}
              <svg width="18" height="18" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className='absolute inset-0 w-full h-full'>
                <path d="M17 1H3C1.89543 1 1 1.89543 1 3V17C1 18.1046 1.89543 19 3 19H17C18.1046 19 19 18.1046 19 17V3C19 1.89543 18.1046 1 17 1Z" 
                stroke="#AAAAAA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
              <svg width="5" height="5" viewBox="0 0 5 5" fill="none" xmlns="http://www.w3.org/2000/svg" className='absolute left-[4px] top-[4px] w-[7px] h-[7px]'>
                <path d="M2.5 4C3.32843 4 4 3.32843 4 2.5C4 1.67157 3.32843 1 2.5 1C1.67157 1 1 1.67157 1 2.5C1 3.32843 1.67157 4 2.5 4Z" 
                stroke="#AAAAAA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
              <svg width="18" height="13" viewBox="0 0 18 13" fill="none" xmlns="http://www.w3.org/2000/svg" className='absolute left-[4px] bottom-[0.5px] w-[20px] h-[14px]'>
                <path d="M17 6L12 1L1 12" stroke="#AAAAAA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
            </div>
