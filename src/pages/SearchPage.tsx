...
const SearchPage = () => {
  ...
  const handleSearch = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && searchKeyword.trim()) {
      e.preventDefault()
      ;(e.target as HTMLInputElement).blur() // 모바일 키보드 닫기
  ...
  }, [hasMore, isLoadingMore, sortedStores.length])
      ...
      <div className='container mx-auto p-4 pt-10 pb-20'>
        ...
        <div className='relative my-6'>
          ...
          <input
            ...
            className='w-full py-3 pl-12 pr-20 rounded-full bg-gray-100 focus:outline-none focus:ring-2 focus:ring-orange-400'
            value={searchKeyword}
          ...
          <button
            ...
            className='absolute right-2 top-1/2 -translate-y-1/2 px-4 py-1.5 bg-orange-500 text-white text-sm rounded-full hover:bg-orange-600 active:bg-orange-700 transition-colors'>
            검색
          </button>
        </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            {hasMore && (
              <div
                ...
                {isLoadingMore ? (
                  <>
                    <SpoonLoader />
                    <p className='mt-4 text-gray-600 text-sm'>더 불러오는 중...</p>
                  </>
                ) : (
                  <div className='h-4' />
                )}
            ...
            {/* 모든 결과 로드 완료 */}
            {!hasMore && sortedStores.length > ITEMS_PER_PAGE && (
              <div className='flex justify-center py-8'>
                <p className='text-sm text-gray-500'>모든 검색 결과를 불러왔습니다</p>
              </div>
            )}
          </>
        )}
